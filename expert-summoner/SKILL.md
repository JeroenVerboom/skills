...
2. **The verdict** — the report's most emphatic block, on a light ground. It announces itself twice: a bordered stamp (`verdict-badge`, 2px ink frame, label-caps in deep terracotta) reading "The verdict", then the recommendation as one large serif statement in italic — the report's only italic heading, the pronounced-judgment register. Below it the single next action as the page's only terracotta fill, and one short rationale line. The block pops through scale and scarcity, never through a dark background: it is the only filled-and-textured 2px-ink frame on the page.
3. **Where the experts agree** — section heading in serif sentence case at 28px, forest green (`sechead--agree`): agreement reads as confidence. Hairline-divided rows; each row the signal in bold sans, the reasoning in one serif line. Keep them factual.
4. **Where the experts clash** — section heading in serif sentence case at 28px, deep terracotta (`sechead--clash`): the tension the reader still owns. Each clash carries a name that states the tradeoff itself ("Speed versus scope", not "Simon disagrees with Hormozi") set in bold sans in ink — never in the section-heading colour, so the two levels stay distinct at a glance — then both sides in one line each, attributed. Never smooth a clash away.
